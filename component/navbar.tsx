...
import { useCurrency, CURRENCIES, CurrencyCode } from "@/app/context/CurrencyContext";
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 md:h-20">
            ...
            <div className="hidden lg:flex items-center gap-1 xl:gap-2">
              {/* Wishlist */}
              <button  title="Wishlist" className="relative p-2 hover:bg-[#f0e8dc] rounded-full transition-all duration-300 hover:scale-110 group">
                <Heart size={18} className="text-[#2C1810] group-hover:text-[#8B6B3D] transition-colors" />
                <span className="absolute -top-0.5 -right-0.5 h-4 w-4 bg-gradient-to-r from-[#C49B5C] to-[#8B6B3D] text-white text-[10px] rounded-full flex items-center justify-center font-bold shadow">0</span>
              </button>

              {/* Cart */}
              <button className="relative p-2 hover:bg-[#f0e8dc] rounded-full transition-all duration-300 hover:scale-110 group">
                <ShoppingBag size={18} className="text-[#2C1810] group-hover:text-[#8B6B3D] transition-colors" />
                <span className="absolute -top-0.5 -right-0.5 h-4 w-4 bg-gradient-to-r from-[#C49B5C] to-[#8B6B3D] text-white text-[10px] rounded-full flex items-center justify-center font-bold shadow ring-2 ring-white">0</span>
              </button>

              {/* Account */}
              <button title="NULL" className="p-2 hover:bg-[#f0e8dc] rounded-full transition-all duration-300 hover:scale-110 group">
                <User size={18} className="text-[#2C1810] group-hover:text-[#8B6B3D] transition-colors" />
              </button>
              ...
              <div className="relative" ref={currencyRef}>
                ...
                >
                  ...
                  <ChevronDown size={13} className={`transition-transform duration-300 ${isCurrencyOpen ? "rotate-180" : ""}`} />
                </button>
                  ...
                  <div className="absolute right-0 mt-2 w-64 bg-white rounded-2xl shadow-2xl border border-[#d4c5a9]/30 overflow-hidden z-50">
                    <div className="px-4 py-3 border-b border-[#d4c5a9]/20 bg-[#f8f3ea] flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-semibold text-[#2C1810]">Select Currency</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] text-gray-400">
                          {isLoading ? "Loading..." : `Updated ${getTimeAgo(lastUpdated)}`}
                        </span>
                        <button title="Refresh Rates" onClick={handleRefreshRates} disabled={isRefreshing} className="p-1 hover:bg-[#e8ddd0] rounded-full transition disabled:opacity-40">
                          <RefreshCw size={12} className={`text-[#C49B5C] ${isRefreshing ? "animate-spin" : ""}`} />
                        </button>
                      </div>
                    </div>
                    {isOffline && (
                      ...
                      {Object.entries(CURRENCIES).map(([code, data]) => (
                        <button
                          ...
                          className={`w-full px-4 py-2.5 text-left text-sm flex items-center justify-between group transition-colors ${
                            currency === code ? "bg-[#f8f3ea]" : "hover:bg-[#f0e8dc]"
                          }`}
                        >
                          <div className="flex items-center gap-3">
                            <span className="text-base w-6 text-center">{data.symbol}</span>
                            <div>
                              <span className={`font-medium ${currency === code ? "text-[#8B6B3D]" : "text-[#2C1810]"}`}>{code}</span>
                              <span className="text-xs text-gray-400 ml-2">{data.name}</span>
                            </div>
                          </div>
                          {currency === code
                            ? <span className="w-2 h-2 bg-[#C49B5C] rounded-full" />
                            : rates && <span className="text-[10px] text-gray-400 opacity-0 group-hover:opacity-100 transition-opacity">{rates[code]?.toFixed(2) || "-"}</span>
                          }
                        </button>
                      ))}
                    </div>
            ...
            <div className="flex lg:hidden items-center gap-1">
              <button className="relative p-2 rounded-full hover:bg-[#f0e8dc] transition-colors">
                ...
                <span className="absolute -top-0.5 -right-0.5 h-4 w-4 bg-gradient-to-r from-[#C49B5C] to-[#8B6B3D] text-white text-[9px] rounded-full flex items-center justify-center font-bold shadow">0</span>
              </button>
              <button className="relative p-2 rounded-full hover:bg-[#f0e8dc] transition-colors">
                <ShoppingBag size={20} className="text-[#2C1810]" />
                <span className="absolute -top-0.5 -right-0.5 h-4 w-4 bg-gradient-to-r from-[#C49B5C] to-[#8B6B3D] text-white text-[9px] rounded-full flex items-center justify-center font-bold shadow ring-2 ring-white">0</span>
              </button>
              <button
              ...
              >
                {open ? <X size={22} className="text-[#2C1810]" /> : <Menu size={22} className="text-[#2C1810]" />}
              </button>
            </div>
          </div>
        </div>
        ...
        >
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-[#d4c5a9]/30 bg-[#faf6ef]">
            <div className="relative w-28 h-9">
              <Image src="/Nav_Logo.png" alt="HCR Textile" fill className="object-contain" sizes="112px" />
            </div>
            <button
          ...
          <div className="flex-1 overflow-y-auto py-3 px-3">
            ...
            {/* Navigation Links */}
            <div className="mb-4">
              <p className="text-[10px] font-semibold text-[#C49B5C] uppercase tracking-widest px-3 mb-2">Menu</p>
              {links.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
            ...
            {/* Account & Wishlist */}
            <div className="my-3">
              <p className="text-[10px] font-semibold text-[#C49B5C] uppercase tracking-widest px-3 mb-2">Account</p>
              <button className="flex items-center gap-3 w-full px-3 py-3 rounded-xl hover:bg-[#f0e8dc] transition-colors text-[15px] font-medium text-[#2C1810] hover:text-[#8B6B3D] group">
              ...
              <button className="flex items-center justify-between w-full px-3 py-3 rounded-xl hover:bg-[#f0e8dc] transition-colors text-[15px] font-medium text-[#2C1810] hover:text-[#8B6B3D]">
                ...
                <span className="bg-gradient-to-r from-[#C49B5C] to-[#8B6B3D] text-white text-[10px] font-bold px-2 py-0.5 rounded-full">0</span>
              </button>
              <button className="flex items-center justify-between w-full px-3 py-3 rounded-xl hover:bg-[#f0e8dc] transition-colors text-[15px] font-medium text-[#2C1810] hover:text-[#8B6B3D]">
                <div className="flex items-center gap-3">
            ...
            {/* Currency Section */}
            <div className="my-3">
              <p className="text-[10px] font-semibold text-[#C49B5C] uppercase tracking-widest px-3 mb-2">Currency</p>
              ...
              <div className="mx-3 mb-2 flex items-center justify-between px-3 py-2.5 bg-[#f8f3ea] rounded-xl border border-[#d4c5a9]/30">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-[#2C1810]">
                    {isLoading ? "Updating..." : `Updated ${getTimeAgo(lastUpdated)}`}
                  </span>
                  {isOffline && <WifiOff size={13} className="text-red-500" />}
                </div>
                <button onClick={handleRefreshRates} disabled={isRefreshing} className="flex items-center gap-1 px-2 py-1 bg-white rounded-lg border border-[#d4c5a9]/40 text-xs text-[#8B6B3D] font-medium disabled:opacity-40 transition-colors hover:bg-[#f0e8dc]">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col items-start leading-tight">
                    <span className="text-sm font-semibold text-[#2C1810]">{currency}</span>
                    <span className="text-[11px] text-gray-400">{CURRENCIES[currency]?.name}</span>
                  </div>
                </div>
                <ChevronDown size={16} className={`text-[#C49B5C] transition-transform duration-300 ${isMobileCurrencyOpen ? "rotate-180" : ""}`} />
              </button>
              ...
              {isMobileCurrencyOpen && (
                <div className="mt-1 mx-0 space-y-0.5 rounded-xl overflow-hidden border border-[#d4c5a9]/20">
                  {Object.entries(CURRENCIES).map(([code, data]) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <span className="text-xl w-7 text-center">{data.symbol}</span>
                        <div className="flex flex-col items-start leading-tight">
                          <span className="font-semibold text-sm">{code}</span>
                          <span className="text-[10px] text-gray-400">{data.name}</span>
                        </div>
                      </div>
                      {currency === code && <span className="w-2.5 h-2.5 bg-[#C49B5C] rounded-full shadow" />}
                    </button>
                  ))}
                </div>
              )}
          ...
          {/* Drawer Footer */}
          <div className="px-5 py-4 border-t border-[#d4c5a9]/30 bg-[#faf6ef]">
            <p className="text-[9px] tracking-[0.2em] text-[#8B6B3D] uppercase font-light text-center">
